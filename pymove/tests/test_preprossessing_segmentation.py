...
def _prepare_df_tid(tid):
    move_df = MoveDataFrame(
        data=list_data, latitude='lat', longitude='lon', datetime='datetime', traj_id='id'
    )
    cols = [
        'id', 'lat', 'lon', 'datetime',
        'dist_to_prev', 'time_to_prev', 'speed_to_prev', tid
    ]
    return move_df, cols
...
def test_by_dist_time_speed():
    ...
    expected = DataFrame(data=[
        ...
        [1, 39.984198, 116.319322, Timestamp('2008-10-23 05:53:06'),
         13.690153134343689, 1.0, 13.690153134343689, 1],
        [2, 39.984224, 116.319402, Timestamp('2008-10-23 05:53:11'),
         nan, nan, nan, 2],
        [2, 39.984224, 116.319402, Timestamp('2008-10-23 05:53:15'),
         0.0, 4.0, 0.000000, 2]
...
def test_by_max_dist():
    ...
    expected = DataFrame(data=[
        [1, 39.984094, 116.319236, Timestamp('2008-10-23 05:53:05'),
         nan, nan, nan, 1],
        [1, 39.984198, 116.319322, Timestamp('2008-10-23 05:53:06'),
         13.690153134343689, 1.0, 13.690153134343689, 2],
        [2, 39.984224, 116.319402, Timestamp('2008-10-23 05:53:11'),
         nan, nan, nan, 3],
        [2, 39.984224, 116.319402, Timestamp('2008-10-23 05:53:15'),
         0.0, 4.0, 0.000000, 3]
    ...
    segmentation.by_max_dist(move_df, max_dist_between_adj_points=0.5, inplace=True)
    assert_frame_equal(move_df, expected)
    assert move_df.shape[1] == 8
...
def test_by_max_time():
    ...
    segmentation.by_max_time(move_df, max_time_between_adj_points=0, inplace=True)
    assert_frame_equal(move_df, expected)
    assert move_df.shape[1] == 8
...
def test_by_max_speed():
    ...
    segmentation.by_max_speed(move_df, max_speed_between_adj_points=0, inplace=True)
    assert_frame_equal(move_df, expected)
    assert move_df.shape[1] == 8
