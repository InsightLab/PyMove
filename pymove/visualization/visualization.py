...
def show_all_features(move_data, figsize=(21, 15), dtype=np.float64, save_fig=True, name='features.png'):
    """
    ...
    """
    try:
        col_float = move_data.select_dtypes(include=[dtype]).columns
        tam = col_float.size
        if tam > 0:
...
def show_traj(move_data, label_tid=TRAJ_ID, figsize=(10, 10), return_fig=True, markers='o', markersize=20):
    """
    ...
    label_tid : String, optional, default 'id'.
        Represents column name 'id'.

    figsize : tuple, optional, default (10,10).
        Represents dimensions of figure.

    return_fig : bool, optional, default True.
        Represents whether or not to return the generated figure.
    ...
    ids = move_data[label_tid].unique()
    
    for id_ in ids:
        move_data_id = move_data[ move_data[label_tid] == id_]
        plt.plot(move_data_id[LONGITUDE], move_data_id[LATITUDE], markers, markersize=markersize)
    if return_fig:
        return fig


def show_traj_id(move_data, tid, label_tid=TRAJ_ID,  figsize=(10, 10)):
    """
    Generate a visualization that show trajectory with id specified.
    ...
    """
    fig = plt.figure(figsize=figsize)
    move_data = move_data[move_data[label_tid] == tid]
    # start point
    plt.plot(move_data.iloc[0][LONGITUDE], move_data.iloc[0][LATITUDE], 'yo', markersize=20)
    # end point
    plt.plot(move_data.iloc[-1][LONGITUDE], move_data.iloc[-1][LATITUDE], 'yX', markersize=20)
    
    if 'isNode' not in move_data:
        plt.plot(move_data[LONGITUDE], move_data[LATITUDE])
        # points
        plt.plot(move_data.loc[:, LONGITUDE], move_data.loc[:, LATITUDE], 'r.', markersize=8)
    else:
        filter_ = move_data['isNode'] == 1
        move_data_nodes = move_data.loc[filter_]
        move_data_points = move_data.loc[~filter_]
        plt.plot(move_data_nodes[LONGITUDE], move_data_nodes[LATITUDE], linewidth=3)
        plt.plot(move_data_points[LONGITUDE], move_data_points[LATITUDE])
        # nodes
        plt.plot(move_data_nodes[LONGITUDE], move_data_nodes[LATITUDE], 'go', markersize=10)
        # points
        plt.plot(move_data_points[LONGITUDE], move_data_points[LATITUDE], 'r.', markersize=8)
    return move_data, fig
