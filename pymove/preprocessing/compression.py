...
def compress_segment_stop_to_point(
    ...
    """
    Compress the trajectories using the stay points in the dataframe. Compreess
    ...
    try:
        ...
        if drop_moves is False:
            move_data.at[
                move_data[move_data[label_stop] is False].index, "lat_mean"
            ] = np.NaN
            move_data.at[
                move_data[move_data[label_stop] is False].index, "lon_mean"
            ] = np.NaN
        else:
            print("...move segments will be dropped...")

        print("...get only segments stop...")
        segments = move_data[move_data[label_stop] is True][
            label_segment
        ].unique()

        sum_size_id = 0
        move_datasize = move_data[move_data[label_stop] is True].shape[0]
        curr_perc_int = -1
        start_time = time.time()
        ...
        print(
            "...Compression time: {:.3f} seconds".format(
...
def compress_segment_stop_to_point_optimizer(
    ...
    """
    Compress the trajectories using the stop points in the dataframe. Compreess
    a segment to point setting lat_mean e lon_mean to each segment.
    ...
    try:
        ...
        if (label_segment in move_data) & (label_stop in move_data):
            ...
            if drop_moves is False:
                lat_mean[
                    move_data[move_data[label_stop] is False].index
                ] = np.NaN
                lon_mean[
                    move_data[move_data[label_stop] is False].index
                ] = np.NaN
            ...
            print(
                "{} or {} is not in dataframe".format(
                    label_stop, label_segment
                )
            )
    except Exception as e:
        raise e
