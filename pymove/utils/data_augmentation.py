...
def generate_target_feature(df_):
    ...
    """
    if LABEL not in df_:
        df_[LABEL] = df_[TRAJECTORY].apply(lambda x: np.int32(x[-1]))
        df_[TRAJECTORY] = df_[TRAJECTORY].apply(lambda x: np.array(x[:-1], dtype=np.int32).tolist())
...
def augmentation_trajectories_df(df_, frac=0.5):
    ...
    for idx1, row1 in progress_bar(df_.iterrows(), total=df_.shape[0]):  
        ...
        for idx2, row2 in df_.iterrows():
            if (row1[LABEL] == row2[LABEL] and
                str(row1[TRAJECTORY]) != str(row2[TRAJECTORY])):
