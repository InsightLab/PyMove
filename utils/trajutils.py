...
def show_trajectories_info(df_, dic_labels=dic_labels):
    ...
    try:
        ...
        print('Number of Rows: {}\nNumber of Ids: {} '.format(df_.shape[0], df_[dic_labels['id']].nunique()))
        print('Trajectories between {} and {}'.format(df_[dic_labels['datetime']].min(), df_[dic_labels['datetime']].max()))
...
def create_update_tid_based_on_id_datatime(df_, dic_labels=dic_labels):
    """
    ...
    try:
        ...
        for idx in ids:
            ...
            if size_id <= 1:
                ...
                raise Exception("{} must have at least 2 GPS points.".format(idx))
            else:
                ...
                sum_size_id += size_id
                curr_perc_int, est_time_str = pru.progress_update(sum_size_id, df_size, start_time, curr_perc_int, step_perc=20)
...
def create_update_dist_time_speed_features(df_, label_id=dic_labels['id'], dic_labels=dic_labels, label_dtype = np.float64, sort=True):
    ...
    """
    try:
        create_update_dist_features(df_, label_id=label_id, dic_labels=dic_labels, label_dtype=label_dtype, sort=sort)
        ...
        for idx in ids:
            ...
            if size_id <= 1:
                ...
                time_prev = (time_ - npu.shift(time_, 1))*(10**-9)
                df_.at[idx, dic_features_label['time_to_prev']] = time_prev

                """ set time_to_next"""
                time_next = (npu.shift(time_, -1) - time_)*(10**-9)
...
def create_update_index_grid_feature(df_, dic_grid=None, dic_labels=dic_labels, label_dtype=np.int64, sort=True):
    ...
    try:
        if dic_grid is not None:
            ...
            print('... inform the grid virtual dictionary\n')
    except Exception as e:
        raise e
...
def clean_duplicates(df_, subset=None, keep='first', inplace=False):
    """
    Return DataFrame with duplicate rows removed, optionally only considering certain columns.
    """
    print('\nRemove rows duplicates by subset')
    tam_drop = df_[df_.duplicated(subset=subset )].shape[0] 
    if tam_drop > 0:
...
def clean_gps_nearby_points(df_, label_id=dic_labels['id'], dic_labels=dic_labels, radius_area=10.0, label_dtype=np.float64):
...
def clean_gps_jumps_by_distance(df_, label_id=dic_labels['id'], jump_coefficient=3.0, threshold = 1, dic_labels=dic_labels, label_dtype=np.float64):
    ...
    try:
        ...
        if dic_features_label['dist_to_prev'] in df_ and dic_features_label['dist_to_next'] and dic_features_label['dist_prev_to_next'] in df_:
            filter_jumpy = (df_[dic_features_label['dist_to_next']] > threshold) & (df_[dic_features_label['dist_to_prev']] > threshold) & \
            ...
            if  idx.shape[0] > 0:
                ...
                df_.drop(index=idx, inplace=True)
                print('...Rows before: {}, Rows after:{}\n'.format(shape_before, df_.shape[0]))
                clean_gps_jumps_by_distance(df_, label_id, jump_coefficient, threshold, dic_labels, label_dtype)      
...
def clean_trajectories_with_few_points(df_, dic_labels=dic_labels, min_points_per_trajectory=2, label_dtype=np.float64):

    create_update_dist_time_speed_features(df_, dic_features_label['tid'], dic_labels=dic_labels, label_dtype=label_dtype)

    if df_.index.name is not None:
        print('\n...Reset index for filtering\n')
        df_.reset_index(inplace=True)

    df_count_tid = df_.groupby(by= dic_features_label['tid']).size()
...
def clean_short_and_few_points_trajectories(df_, dic_labels=dic_labels, min_trajectory_distance=100, min_points_per_trajectory=2, label_dtype=np.float64):
    ...
    clean_trajectories_with_few_points(df_, dic_labels, min_points_per_trajectory)
    ...
    df_agg_tid = df_.groupby(by=dic_features_label['tid']).agg({dic_features_label['dist_to_prev']:'sum'})
    ...
    shape_before_drop = df_.shape
    idx = df_[ df_[dic_features_label['tid']].isin(tid_selection) ].index
    if idx.shape[0] > 0:
        tids_before_drop = df_[dic_features_label['tid']].unique().shape[0]
        df_.drop(index=idx, inplace=True)
        print('\n...Tids - before drop: {} - after drop: {}'.format(tids_before_drop, df_[dic_features_label['tid']].unique().shape[0]))
        print('\n...Shape - before drop: {} - after drop: {}'.format(shape_before_drop, df_.shape))
